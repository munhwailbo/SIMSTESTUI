...
Namespace ubiLease.CommonPopup
    ...
    Public Class frmCodeName
        ...
        'Private McodeDivisionHangulName As String = "" '차재윤 복원
        ...
        Private Sub DynamicControlAdd(ByVal vMcodeDivisionHangulName As String)
            ...
            If vMcodeDivisionHangulName = "거래처 명" Then
                popupmanager.dynamicDesign(Me, 0, 2)
                '
                popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblBizNumber", "사업자", 56, 0, 1, "")
                popupmanager.ControlAdd(Me, ubiControlType.txt, "txtBizNumber", "", 110, 1, 1, "format(&&&-&&-&&&&&);")

                popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblJuminNumber", "주민번호", 56, 2, 1, "")
                popupmanager.ControlAdd(Me, ubiControlType.txt, "txtJuminNumber", "", 150, 3, 1, "format(&&&&&&-&&&&&&&);")

                popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblRepresentativeName", "대표자명", 56, 0, 2, "")
                popupmanager.ControlAdd(Me, ubiControlType.txt, "txtRepresentativeName", "", 110, 1, 2, "")

                'popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblCustomerGroupingCode", "그룹분류", 56, 2, 2, "")
                'popupmanager.ControlAdd(Me, ubiControlType.cbo, "cboCustomerGroupingCode", "", 150, 3, 2, "code(거래처 분류|선택하세요);")
            ElseIf vMcodeDivisionHangulName = "기표 거래처 명" Then
                popupmanager.dynamicDesign(Me, 0, 2)
                '
                popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblBizNumber", "사업자", 56, 0, 1, "")
                popupmanager.ControlAdd(Me, ubiControlType.txt, "txtBizNumber", "", 110, 1, 1, "format(&&&-&&-&&&&&);")
                ...
                'popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblCustomerGroupingCode", "그룹분류", 56, 2, 2, "")
                'popupmanager.ControlAdd(Me, ubiControlType.cbo, "cboCustomerGroupingCode", "", 150, 3, 2, "code(거래처 분류|선택하세요);")
            ElseIf vMcodeDivisionHangulName = "그룹 계열사 명" Then
                popupmanager.dynamicDesign(Me, 0, 2)
                '
                popupmanager.ControlAdd(Me, ubiControlType.lbl, "lblBizNumber", "사업자", 56, 0, 1, "")
                popupmanager.ControlAdd(Me, ubiControlType.txt, "txtBizNumber", "", 110, 1, 1, "format(&&&-&&-&&&&&);")
        ...
        Public Sub Initialize(ByVal AcodeDivisionHangulName As String, Optional ByVal Awhere As String = "" _
            ...
            '& input output io (i:input/o:output); 필수 입력 y/n; 숨기기 y/n; 최대 문자 길이; popup code

            'If _
            '    McodeDivisionHangulName(0) = "계약자 명" _
            '    Or McodeDivisionHangulName(0) = "관리비 계약자 명" _
            '    Or McodeDivisionHangulName(0) = "중장기 본부 명" Then
            ...
            '       and CodeDivisionHangulName  = '지국장코드' --@codeDivisionHangulName  
        ...
        Private Sub frmCodeName_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
            ...
            '우편 번호 일때
            'If Not (McodeDivisionHangulName(0) = "우편번호" And Len(Me.txtCode.Text) + Len(Me.txtName.Text) = 0) Then
